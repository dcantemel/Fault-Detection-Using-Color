...
%         if sembMap(i,j)<0.7 && idx1(i,j) == 1
        if  idx(i,j) == 1
            idx_a(i,j) = 1;
        elseif sembMap(i,j) < 0.7 && idx(i,j) >= 2
            idx_a(i,j) = 1;
         end
    end
end
...
bw1 = bwmorph(skeleton(faultReg2).*ave_discon>6,'skel',Inf);
% figure,imshow([bw0, bw1],'border','tight');
